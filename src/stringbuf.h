...
static inline struct StringBuf *StringBuf_alloc(size_t const buf_capacity) {
...
static inline void StringBuf_add_string(struct StringBuf *buf, const char *ptr,
                                        size_t len) {
  assert(buf->buf_capacity - buf->buf_length >= len);
  for (uint16_t i = buf->buf_length, j = buf->buf_length + len; i < j; ++i) {
    buf->buf[i] = *ptr++;
  }
  buffer->buf_length += len;
}
