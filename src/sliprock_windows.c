...
struct SliprockConnection {
  ...
  struct pipe pipe;
  char passwd[32];
  wchar_t *file_path; //< The path – must be passed to free() when no
                      //longer needed
  char name[];
};
...
#endif
static BOOL sliprock_windows_init() {
  PVOID lpContext;
  BOOL fIsPending;
  BOOL fStatus =
      InitOnceBeginInitialize(&MyOnceInit, 0, &fIsPending, &lpContext);
  if (!fStatus) {
    return FALSE;
  }
  if (!fIsPending)
  ...
  static const wchar_t *get_fname(const char *srcname, size_t len, int pid,
    ...
    if (homelen == 0)
      return NULL;
    wchar_t *path =
        calloc(20 /* length of UINT64_MAX as decimal */ +
                   homelen /* length of home directory */ + len +
                   sizeof "\\.sliprock\\..sock" + 4 /* size of \\?\ */,
               2);
    if (path == NULL)
      return NULL;
    ...
    *innerlen = newhomelen + res;
    if (len > INT_MAX)
      abort();
    size_t newlen =
        MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, srcname, len,
                            path + newhomelen + res, len);
  ...
  struct SliprockConnection *sliprock_socket(const char *const name,
    ...
    if (NULL == connection)
      goto fail;
    connection->path = get_fname(name, namelen, GetCurrentProcessId(),
                                 &innerlen, &connection->pathlen);
    ...
    hFile =
        CreateFileW(connection->path, GENERIC_WRITE, 0, &sec_attributes,
                    CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (INVALID_HANDLE_VALUE == hFile)
      goto fail;
    DWORD w;
    WriteFile(hFile, buf, &w, NULL);
    CloseHandle(hFile);
