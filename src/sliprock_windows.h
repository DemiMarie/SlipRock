...
static void sliprock_strerror(void) {
  ...
  if (buflen) {
    if (!WriteConsoleW(GetStdHandle(-12), buf, buflen, &dummy, NULL)) {
      ...
      if (!buf)
        goto done;
      WideCharToMultiByte(CP_UTF8, 0, buf, buflen, bytebuf, size, NULL, NULL);
...
sliprock_get_handle_for_connection(struct SliprockConnection *connection) {
  ...
  connection->hPipe = CreateNamedPipeW(
      connection->pipename,
      PIPE_ACCESS_DUPLEX | (NULL == hPipe ? FILE_FLAG_FIRST_PIPE_INSTANCE : 0),
...
SLIPROCK_API int sliprock_accept(struct SliprockConnection *connection,
  ...
  /*
   * It is absolutely imperative that there be ONLY ONE instance of the named
   * pipe open but not connected when ConnectNamedPipe() is called.  Otherwise,
   * Windows will pick an arbitrary one.  This causes ConnectNamedPipe() to hang
   * intermittently, causing problems that are terribly difficult to debug!
   *
   * The solution is to wait until ConnectNamedPipe() returns to make the next
   * CreateNamedPipe() call.  This bug took a very long time to fix.
   */
  DWORD written_this_time, written = 0;
  HANDLE hPipe = connection->hPipe;
  int err;
  sliprock_trace("sliprock_accept() called.  Getting connection handle.\n");
...
SLIPROCK_API int sliprock_connect(const struct SliprockReceiver *receiver,
  ...
  *handle = (SliprockHandle)INVALID_HANDLE_VALUE;
  ...
  if ((read = sliprock_read_all(hPipe, pass, sizeof pass)) != sizeof pass) {
#ifdef SLIPROCK_TRACE
    sliprock_strerror();
